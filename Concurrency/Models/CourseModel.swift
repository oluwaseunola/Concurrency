...
struct CoursesCollection : Decodable {
    ...
    init(_ coursesCollection: CourseData?){
        ...
        self.courses = coursesCollection?.courses.map({ Course($0)
        }) ?? []
...
struct Course : Decodable, Identifiable{
    ...
    init(_ course: CourseData.Course?){
        ...
        self.image = course?.image?.url ?? ""
        self.sections = SectionCollection(course?.sectionCollection)
...
struct SectionCollection : Decodable {
    ...
    let sectionCollection : [Section]
    
    init(_ sectionCollection: CourseData.Course.SectionCollection?){
        
        self.sectionCollection = sectionCollection?.sections.map({
            Section($0)
        }) ?? []
    }
